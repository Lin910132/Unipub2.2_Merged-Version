...
//New, Favorite, Rank, Hot, All Time Hot
    ...
    override func viewDidAppear(animated: Bool) {
        if (fromPost == true){
            ...
            self.tabBarButtonClicked(buttons[2])
        }
    }
    ...
    func loadTableViews(){
        ...
        self.scrollView.frame = CGRectMake(0, scrollView.frame.origin.y, mainWidth, mainHeight)
        print("scrollViewFrame - \(self.scrollView.frame)")
        let mainFrame = self.scrollView.frame
        print("mainFrame - \(mainFrame)")
        
        for (var i = 0; i < 5; i++){
            
            let view:UIView = UIView(frame: CGRectMake(mainWidth * CGFloat(i), 0, mainWidth, mainFrame.height))
        ...
        self.scrollView.contentSize = CGSizeMake(mainWidth * 5, mainFrame.size.height)
    }
    ...
    func loadData(type:Int)
    {
        ...
        YRHttpRequest.requestWithURL(url,completionHandler:{ data in
            ...
            if self.page[type] == 1 {
                self.dataArray[type] = NSMutableArray()
            }
            ...
            {
                var isExist:Bool = false
                for item in self.dataArray[type]
                {
                    let oldId = data["id"] as! Int
                    let newId = item["id"] as! Int
                    if  oldId == newId
            ...
            (self.tableArray[type] as! UITableView).reloadData()
        })
    }
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int{
        ...
        print("table row count =\(tableIndex) & \(self.dataArray[tableIndex].count)")
...
/*
        for (var i = 0; i < 5; i++){
            ...
            let titleWidth = button.titleLabel?.frame.size.width
            
            button.frame = CGRectMake(basePos.x, basePos.y, titleWidth! + 2 * space, button.frame.height)
            basePos.x += titleWidth! + 2 * space
        }
*/
